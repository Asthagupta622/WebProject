...
function hideAllDashboardSections() {
  ...
  document.getElementById('client-create-page')?.style?.setProperty('display', 'none');
...
function initializeSignInLinks() {
  document.querySelectorAll('.sign-in-link').forEach(link => {
    link.addEventListener("click", function (e) {
      ...
      if (!email || !password) {
        return alert("Please enter Email and Password to sign in.");
      }

      signupStarted = true;
      showStep(wrapper, 3);
      document.querySelector(".breadcrumb").textContent = `Signin > ${capitalize(role)} > Details`;
      document.querySelector(".breadcrumb-step-3")?.style?.setProperty('display', 'block');

      // ✅ NEW: Show dashboard just like OTP process
      document.querySelector(".container").style.display = "none";
...
function showClientCreatePage() {
  hideAllDashboardSections(); // hide all other pages
  document.getElementById("client-dashboard").style.display = "flex"; // keep main layout visible
  document.getElementById("client-create-page").style.display = "block"; // show create task page
  document.getElementById("client-dashboard").classList.add("white-bg");
